...
import { ShoppingCart, Minus, Plus, Trash2, ExternalLink, Loader2, FileText } from "lucide-react";
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-sm mt-2">Browse the library and add some printables</p>
              </div>
            </div>
          ) : (
            <>
                ...
                <div className="space-y-4">
                  {items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.variantId}
                        className="flex gap-3 p-3 border-2 border-foreground bg-card"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-display text-sm uppercase truncate leading-tight">
                            {item.product.node.title}
                          </h4>
                          <p className="text-xs text-muted-foreground mt-1">
                            {item.selectedOptions.map((o) => o.value).join(" • ") || "Digital PDF"}
                          </p>
              ...
              <div className="flex-shrink-0 space-y-3 p-6 border-t-2 border-foreground bg-secondary">
                ...
                <p className="text-xs text-center text-muted-foreground">
                  Instant download after payment • 30-day refund
                </p>
              </div>
            </>
          )}
