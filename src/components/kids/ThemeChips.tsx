  ...
  return (
    <div className="flex flex-wrap gap-2">
      {themes.map((t) => {
        ...
        return (
          <button
            key={t.slug} type="button" onClick={() => toggle(t.slug)} aria-pressed={active}
            className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border-2 border-foreground text-xs sm:text-sm font-sans transition-all ${
              active ? "bg-accent text-accent-foreground shadow-brutal" : "bg-background hover:bg-secondary"
            }`}
          >
      ...
      {value.length > 0 && (
        <button type="button" onClick={() => onChange([])} className="px-3 py-1.5 text-xs font-mono uppercase underline hover:no-underline">
          Clear themes
        </button>
      )}
    </div>
