              ...
              <Link key={s.id} to={`/kids/coloring/${s.id}`} className="group block">
                <div className="aspect-square bg-muted overflow-hidden rounded-lg">
                  {(s.thumbnail_url || s.cover_url) && (
                    <img src={s.thumbnail_url || s.cover_url!} alt={s.title} loading="lazy" className="w-full h-full object-cover object-center group-hover:scale-[1.03] transition-transform duration-500" />
                  )}
                </div>
