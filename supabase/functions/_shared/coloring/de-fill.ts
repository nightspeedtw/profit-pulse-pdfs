...
/**
 * Compute a pixel-erosion mask on a 1-bit input.
 *
 * INPUT  `mask`  — 1 = target pixel (black / shadow), 0 = other.
 * OUTPUT `keep`  — 1 = pixel to KEEP as target (within ring_px of the
 *                  cluster boundary), 0 = pixel to WHITEN.
 *
...
export function computeDeFillKeepMask(
  ...
  height: number,
  mask: Uint8Array,
  clusterMask: Uint8Array, // 1 = pixel belongs to an oversized cluster to be processed
  ringPx: number,
): Uint8Array {
  const total = width * height;
  // distance = min steps to a non-target neighbour, capped at ringPx+1.
  // Seed: any target pixel adjacent to a non-target (or off-grid) neighbour
  // gets distance 1. Propagate outward via multi-source BFS.
  const dist = new Int32Array(total);
  for (let i = 0; i < total; i++) dist[i] = mask[i] ? -1 : 0; // 0 = non-target
  const queue: number[] = [];
  ...
  // A pixel is KEPT if it is a target AND either it is not in a de-fill
  // cluster (leave normal line-art alone) OR its distance <= ringPx.
...
interface AnalyzedClusters {
  mask: Uint8Array;              // 1 = black target (line + fill)
  shadowMask: Uint8Array;        // 1 = mid-gray (shadow) target
  oversizedBlack: Uint8Array;    // 1 = pixel in a black cluster exceeding max_cluster_ratio
  oversizedShadow: Uint8Array;   // 1 = pixel in a shadow cluster exceeding shadow_cluster_ratio
  clustersDefilled: number;
  shadowsCleared: number;
  largestClusterBefore: number;
}

function tagOversizedClusters(
  width: number,
  height: number,
  mask: Uint8Array,
  minSize: number,
